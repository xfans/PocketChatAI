...
class ChatRepository {
  ...
  Future<void> sendMessage(ModelProvider provider,String content,{bool isWebSearch = false}) async {
    ...
    list.add(ChatCompletionMessage.user(
    ...
    try {
      ...
      var aiResponse = await OpenAiCompatible(provider).completion(list,tools: mcpTools);
      ...
      if(toolCalls != null && toolCalls.isNotEmpty){
        for(var toolCall in toolCalls){
          ...
          final functionResult = await MCPManager.instance.execute(functionCall.name,arguments);
          print("toolCallResponse:${json.encode(functionResult)}");
          list.add(ChatCompletionMessage.tool(
        ...
        aiResponse = await OpenAiCompatible(provider).completion(list,tools: mcpTools);
        print("aiResponse2:${aiResponse.choices.first.message.content}");
      }
      final aiMessage = Message.ai(
        content: aiResponse.choices.first.message.content ?? 'No response from AI',
        sessionId: session.id
      );
